...
VPN_CLIENT_CONF="${BUILDS_BASE}/vpn-client-confs/client-public.conf"
# ------------------------------------------------------------------- #

if [[ -z "${SCRIPT_DIR}/mybuilds.conf" ]]; then
    read -p "[+] Declare your BUILDS folder for this and future build efforts: " -i "${HOME}/builds" -e BUILDS_BASE
    mkdir -p "${BUILDS_BASE}"
    echo "BUILDS_BASE=\"${BUILDS_BASE}\"" > "${SCRIPT_DIR}/mybuilds.conf"
else
    source "${SCRIPT_DIR}/mybuilds.conf"
fi
...
if [ $? -ne 0 ]; then
	...
	md5sum "${ISO_FILE}" > "/var/www/html/iso/${BUILD_NAME}.md5"
	# -u = means only copy if source file is newer than destination file
	cp -u "${ISO_FILE}" "/var/www/html/iso/${BUILD_NAME}.iso"
fi
